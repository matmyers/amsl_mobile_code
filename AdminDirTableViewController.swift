...
//
//  View Controller for Admin Directory
//
...
// enum to determine whether user is searching by person's name or title
enum selectedScope: Int {
    case name = 0
    case title = 1
}

class AdminDirTableViewController: UITableViewController, UISearchBarDelegate {
    
    // set url to where json data is read
    private final let urlString = "https://matmyers.github.io/amsl_mobile/mobileAppAdminDir.json"
    ...
    func searchBarSetup() {
        ...
        searchBar.scopeButtonTitles = ["Name","Title"]
    ...
    func filterTableView(ind: Int, text: String) {
        ...
        case selectedScope.name.rawValue:
            //fix if not searching when backspacing
        ...
        case selectedScope.title.rawValue:
            //fix if not searching when backspacing
            personArray = filteredArray.filter({ (mod) -> Bool in
                return mod.job.lowercased().contains(text.lowercased())
            })
            self.tableView.reloadData()
        default:
            print("no type")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "AdminDirectoryTableViewCell") as! AdminDirectoryTableViewCell
